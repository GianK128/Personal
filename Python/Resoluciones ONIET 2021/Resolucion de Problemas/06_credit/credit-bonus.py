...
def validate_cc_csv():
    # Inicializar un diccionario donde ponemos los datos para los csv
    new_rows = {
        'INVALIDAS': []
    }

    # Poblar (?) el dict con listas vacias
    for key in CC_CONDITIONS.keys():
        new_rows[key] = []
    ...
    with open(f"{cc_filepath}/tarjetas.csv") as f:
        ...
        for row in reader:
            ...
            if not validate_cc(cc_num):
                new_rows['INVALIDAS'].append({'TARJETA': cc_num, 'RED': 'INVALIDAS'})
                continue

            # Obtener la red bancaria o si es invalida
            red = identify_cc(cc_num)

            # Si es valida, agregarla segun su red o si no pertenece a ninguna.
            new_rows[red].append({'TARJETA': cc_num, 'RED': red})
        
        # Abrir nuevo archivo csv y meterle los datos luego de la validacion
        for key, new_val in new_rows.items():
            with open(f"{cc_filepath}/tarjetas-{key}.csv", "w", newline='') as nf:
